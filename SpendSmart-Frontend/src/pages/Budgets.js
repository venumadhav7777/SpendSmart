...
function Budgets() {
  ...
  const [newBudget, setNewBudget] = useState({ name: '', category: '', limit: '', period: '' });
  ...
  const handleCreateBudget = async () => {
    ...
    try {
      ...
      setNewBudget({ name: '', category: '', limit: '', period: '' });
      loadBudgets();
    } catch (err) {
      ...
      <SectionCard sx={{ mb: 3 }}>
        ...
        <Box sx={{ mb: 2, display: 'flex', flexWrap: 'wrap', gap: 2 }}>
          ...
          <TextField
            ...
            sx={{ flex: '1 1 150px' }}
          />
          <Button variant="contained" onClick={handleCreateBudget} sx={{ alignSelf: 'center' }}>
            Create Budget
          </Button>
        </Box>
